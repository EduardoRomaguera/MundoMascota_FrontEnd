...
const aprobarProveedoresPendientes = async(pcorreo) => {
    await axios({
        ...
        Swal.fire({
            'icon': 'success',
            'title': 'El cliente se desbloqueó correctamente',
            'text': response.msj
        }).then(() => {
            window.location.href = 'P99-reporte-usuarios-baneados.html';
        });
    }).catch((error) => {
        Swal.fire({
            'title': 'Error',
...
const mostrarTablaProveedoresPendientes = async => {
    ...
    listaProveedores.forEach(proveedor => {
        if (proveedor.nombre.toLowerCase().includes(filtro) && (proveedor.estado == "bloqueado")) {
            ...
            btnAceptar.innerText = 'Desbloquear';

            btnAceptar.addEventListener('click', () => {
                Swal.fire({
                    'icon': 'warning',
                    'text': '¿Está seguro que desea desbloquear al cliente?',
